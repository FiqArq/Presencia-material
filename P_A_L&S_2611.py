...
#cap = cv2.VideoCapture("../Video Andamio/3H1m.mp4")
#(1920, 1080)
#640,360
...
#AR conexion serial con arduino y zonificacion
ser = serial.Serial("COM3",9600, timeout=5)
ser2 = serial.Serial("COM4",9600, timeout=5)
time.sleep(5)

mapeo = cv2.imread("../Video Andamio/mapeoblack.png")

###probando el seguimiento de linea
    ...
    for result in resultsTracker:
        ...
        for i in range(1, len(data_deque[Id])):
            ...
            for vistas_combinadas in [camara_musica_bn, mapeo]:
                cv2.line(vistas_combinadas, data_deque[Id][i - 1], data_deque[Id][i], (colores[int(Id) % len(colores)]),
                         (grosor if vistas_combinadas is camara_musica_bn else 2))
        ###########################################################################
